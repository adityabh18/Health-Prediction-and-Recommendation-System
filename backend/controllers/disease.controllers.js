...
export const predictGeneralDesease = async (req, res) => {
  try {
    ...
    const localName = recommendations?.localName || "";
...
export const predictHeartDisease = async (req, res) => {
  try {
     ...
     * required fields
     */
    if (!Sex || !AgeCategory || !HeightInMeters || !WeightInKilograms || !BMI) {
      return res.status(400).json({
     ...
     * Call ML Server
     */
    const AI_response = await axios.post(mlServer + "/predict-heart", {
      Sex,
      AgeCategory,
      HeightInMeters,
      WeightInKilograms,
...
export const predictKidneyDisease = async (req, res) => {
  try {
    const userId = req.user?.id || null;
    let { age, bp, sg, al, su, bgr, bu, sc, hemo, htn, dm, appet } = req.body;
